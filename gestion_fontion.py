...
cursor.execute('INSERT INTO Facture (id, id_Clients, DATE_EMISSION, MONTANT_TOTAL) VALUES (%s,%s,%s,%s)', (addl[0], addl[1], addl[2], addc[3]))
logging.info("lancement de la fonction d'ajout de facture ")
return 0

def add_ligne(cursor):
#Ajouter une facture détaille à la bdd
cursor.execute('INSERT INTO Lignes (id_Facture, NOM_DU_PRODUIT, REFERENCE, QUANTITE, PRIX_UNITE) VALUES (%s;%s,%s,%s,%s)', (addf[0],addf[1],addf[2],addf[3], addf[4]))
...
cursor.execute("""SELECT id FROM Facture WHERE '%s' """, (rechf))
...
cursor.execute("""UDPDATE Clients SET '%s' = '%s' WHERE '%s' """, (modc[0], modc[1], modc[2]))
...
for i in cursor.fetchall():
    factures = (f'id{i[0]} id_Clients{i[1]} DATE_EMISSION{i[2]} MONTANT_TOTAL{i[3]}')
    print(factures)
